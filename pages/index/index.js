...
Page((e = {
  ...
  goliaos: function() {
    wx.navigateTo({
      url: "/pages/commission/list/index"
    });
  },
  onLoad: function(t) {

    // console.log(wx.getStorageSync('isread'))
    t = t || {};

    var a = this;

    console.log(a.data.isread)
    ...
    setTimeout(function() {
      ...
      console.log(t)
      // if (t.diypage.M1506332568231.data){
      //   for (var i = 0; i <= t.diypage.M1506332568231.data.length; i++) {
      //     for (var g = 0; i <= t.diypage.M1506332568231.data[i].level.length; i++) {
      //       if (i.getCache("olevel", e.level) == ) {
  ...
  onShow: function() {
    ...
    console.log(i.globalData.isread)
    ...
    t.data.pages && "" != t.data.pages.title && (o = t.data.diytitle), t.data.pages && wx.setNavigationBarColor({
  ...
  bailichangeimg: function(t) {
    if (this.data.diypages && t) {
      var a = t.currentTarget.dataset.type,
        e = this.data.diypages.items[a].data;
