...
Page({
  data: {
    ...
    ystatusp: '1',
    'list.invoice_info.title': '',
    'list.invoice_info.number': '',
    'list.invoice_info.address': '',
    'list.invoice_info.bank': '',
    'list.invoice_info.mobile': '',
    'list.invoice_info.bank_num': '',
    // 'list.invoicename':"",
    ystatusps: "",
    goodslist: {},
    data: {
    ...
    // 'list.invoice_info.title': false,
    // 'list.invoice_info.number':false,
    merchid: 0,
    showPicker: !1,
    pvalOld: [0, 0, 0],
    pval: [0, 0, 0],
  ...
  onShow: function () {
    ...
    //   console.log(t.couponcount)
    //   i.setData({
    //     list: t
    //   })
    // })
    var opsfapiao = e.getCache("opsfapiao")
    console.log(e.getCache("opsfapiao"))
    i.setData({
      'list.invoice_info.title': opsfapiao.invoice_name,
      'list.invoice_info.number': opsfapiao.invoice_num,
      'list.invoice_info.address': opsfapiao.address,
      'list.invoice_info.bank': opsfapiao.bank,
      'list.invoice_info.mobile': opsfapiao.mobile,
      'list.invoice_info.bank_num': opsfapiao.bank_num,
    })
  ...
  ycfp: function (e) {
    ...
    console.log(this.data.ystatusps)
  },
  ycfps: function (e) {
    this.setData({/////

      ystatusps: e.detail.value
  ...
  caculate: function (t) {
    ...
    e.data.data && 0 != e.data.data.couponid && (i = e.data.data.couponid), a.post("order/create/caculate", {
      ...
      console.log(a)
      e.setData({
        couponcount: a.couponcount
      })
      console.log(e.data.couponcount)
    ...
    console.log(this.data.list.goods[0].goods)
    getApp().setCache("goodsInfo", {
      goodslist: this.data.list.goods[0].goods,
      merchs: this.data.list.merchs
    }, 1800);
  },
  submit: function () {
    var t = this.data,
      ...
      d = t.goods.giftid;
    console.log(t)
    console.log(t.giftid)
    if (t.ystatusp == "2") {
      console.log(this.data.ystatusp)
      if (!this.data.list.invoicename || this.data.list.invoicename == "") {
        wx.showToast({
          title: '请填写发票',
          icon: 'none'
        })
    ...
    // var opsfapiao = getApp().getCache("opsfapiao")
    // console.log(getApp().getCache("opsfapiao"))
    // e.setData({
    //   'list.invoice_info.title': opsfapiao.invoice_name,
    //   'list.invoice_info.number': opsfapiao.invoice_num
    // })
    if (0 == this.data.goods.giftid && 1 == this.data.goods.gifts.length && (d = this.data.goods.gifts[0].id), !t.submit && r.verify(this, i)) {
      ...
      // console.log(t.ystatusp)
      // console.log(t.ystatusp)
      t.list.carrierInfo = t.list.carrierInfo || {};
      var o = {
        is_invoice: t.ystatusp,
        invoice_type: t.ystatusps,
        invoice_address: t.list.invoice_info.address,
        invoice_bank: t.list.invoice_info.bank,
        invoice_mobile: t.list.invoice_info.mobile,
        invoice_bank_num: t.list.invoice_info.bank_num,
        invoice_name: t.list.invoice_info.title,
        invoice_num: t.list.invoice_info.number,
  ...
  confirminvoice: function () {
    ...
    // t.invoicename = a, t.invoice_info.title + t.invoice_info.number ? t.invoice_info.company && !t.invoice_info.number ? i.toast(this, "请填写税号") : this.setData({
    //   list: t,
    //   // invoicepicker: !1
    // }) : i.toast(this, "请填写发票抬头");
    console.log(this.data.ystatusps)
    if (this.data.ystatusps == "普通发票") {

      if (this.data.list.invoice_info.title == "") {
        i.toast(this, "请填写发票抬头")
      } else if (this.data.list.invoice_info.number == "") {
        i.toast(this, "请填写纳税人识别号")
      } else {
        this.setData({
          'list.invoicename': this.data.list.invoice_info.title,
          invoicepicker: !1
        })
      }
    }

    if (this.data.ystatusps == "专用发票") {
      if (!this.data.list.invoice_info.address || this.data.list.invoice_info.address == "") {
        i.toast(this, "请填写地址")
      } else if (!this.data.list.invoice_info.mobile || this.data.list.invoice_info.mobile == "") {
        i.toast(this, "请填写手机号")
      } else if (!this.data.list.invoice_info.bank || this.data.list.invoice_info.bank == "") {
        i.toast(this, "请填写银行名称")
      } else if (!this.data.list.invoice_info.bank_num || this.data.list.invoice_info.bank_num == "") {
        i.toast(this, "请填写银行账号")
      } else if (!this.data.list.invoice_info.title || this.data.list.invoice_info.title == "") {
        i.toast(this, "请填写发票抬头")
      } else if (!this.data.list.invoice_info.number || this.data.list.invoice_info.number == "") {
        i.toast(this, "请填写纳税人识别号")
      } else {
        console.log(1)
        console.log(this.data.list.invoice_info.address)
        this.setData({
          invoicepicker: !1,
          'list.invoicename': this.data.list.invoice_info.title,
          // 'list.invoicename': '抬头:' + '('+this.data.list.invoice_info.title+')' + '识别号:' + '('+this.data.list.invoice_info.number+')',
        })
      }
    }
